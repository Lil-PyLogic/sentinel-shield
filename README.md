...
[Features](#-features) · [Installation](#-installation) · [Usage](#-usage) · [Detection Methodology](#-detection-methodology) · [Architecture](#-architecture) · [Security](#-responsible-disclosure) · [Contributing](#-contributing) · [License](#-license)
...
- **🛡️ Safe by default** — symlink cycles are detected and pruned; FIFO/devices/sockets are skipped; broken paths warn instead of crash.
...
A live HTML version of this diagram is at [`architecture.html`](./architecture.html) in this repo.
...
Sentinel Shield is a **defensive** tool. It analyzes files statically and never executes them. We take abuse — and the safety of researchers, victims, and the wider community — seriously.

### What this tool is for

- ✅ Scanning files you own or are authorized to analyze (your own systems, customer systems under contract, malware samples in a lab environment).
- ✅ Triaging suspicious files during incident response.
- ✅ Adding an extra layer of defense on top of an existing EDR/AV pipeline.
- ✅ Educational use and security research.

### What this tool is **not** for

- ❌ Scanning systems you do not own or are not authorized to test.
- ❌ Building offensive tooling, weaponizing detection bypasses, or evading other people's EDR products.
- ❌ Harassment, extortion, or any illegal activity.

### Coordinated disclosure

If you discover a **vulnerability in Sentinel Shield itself** (a bypass, a path-traversal flaw, an RCE, an information leak), please report it privately before disclosing publicly:

- **Email:** `security@sentinel-shield.example` (PGP key on request)
- **Subject prefix:** `[SECURITY]`
- **Response target:** within 72 hours

We follow the standard 90-day coordinated-disclosure window. Reporters who follow responsible disclosure will be credited in the release notes (unless you prefer to stay anonymous).

### Misuse

If you believe Sentinel Shield is being used to harm others, contact the same address above with whatever evidence you can safely provide. We will not help with retaliation, but we will cooperate with lawful investigations.
